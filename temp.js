...
var r_3_XX_;
function preload(){
	game.load.image('e_1_XX_','assets/sprites/newArt/Circle/Circle_grid2.png');
...
function create(){
	...
	game.input.onDown.add(click_to_spin_entity_e_2_XX__XX__XX_PressedHandler, this);
	game.time.events.loop(Phaser.Timer.SECOND*3, pingpong_entity_e_3_XX__XX__XX__pingpong_entity_e_3_XX__XX__XX_Listener, this);
	...
	var barConfig2 = createProgressBarConfig(r_3_XX_, 2, labels['r_3_XX_']);
	this.resourceBar2 = new HealthBar(this.game, barConfig2)
	addBarLabel(barConfig2, 2, labels['r_3_XX_']);
	
	var percent2 = r_3_XX_/10;
	percent2 = percent2 * 100;
	this.resourceBar2.setPercentNow(percent2);
	};
...
function update(){
	...
	if (!game.physics.arcade.overlap(addedEntities['e_1_XX_'],addedEntities['e_3_XX_'],null,null, this)){o_1_XX_NotOverlapHandler();}
	game.physics.arcade.overlap(addedEntities['e_1_XX_'],addedEntities['e_3_XX_'],o_2_XX_OverlapHandler,null, this);
	if (!game.physics.arcade.overlap(addedEntities['e_1_XX_'],addedEntities['e_2_XX_'],null,null, this)){o_3_XX_NotOverlapHandler();}
	game.physics.arcade.overlap(addedEntities['e_1_XX_'],addedEntities['e_3_XX_'],o_4_XX_OverlapHandler,null, this);
	...
	var percent2 = r_3_XX_/10;
	percent2 = percent2 * 100;
	this.resourceBar2.setPercent(percent2);
...
function click_to_spin_entity_e_2_XX__XX__XX_PressedHandler(){
	if(r_1_XX_<=0){
		r_3_XX_=r_3_XX_-6/2;

	r_1_XX_=r_1_XX_+6/2;

	addedEntities['e_2_XX_'].forEach(function(item){item.angle += 45;}, this);
		}
};
...
function o_1_XX_NotOverlapHandler(){
	...
	if (typeof e1 !== 'undefined' && e1.key === 'e_1_XX_'){
		e1.health = e1.health+5/40;
	}
	if (typeof e2 !== 'undefined' && e2.key === 'e_1_XX_'){
		e2.health = e2.health+5/40;
	}

	r_1_XX_=r_1_XX_-6/40;

	r_3_XX_=r_3_XX_+3/40;
...
function o_3_XX_NotOverlapHandler(){
	...
	if (typeof e1 !== 'undefined' && e1.key === 'e_1_XX_'){
		e1.health = e1.health+2/40;
	}
	if (typeof e2 !== 'undefined' && e2.key === 'e_1_XX_'){
		e2.health = e2.health+2/40;
	}

	r_1_XX_=r_1_XX_-8/40;

	r_3_XX_=r_3_XX_-6/40;
...
function pingpong_entity_e_3_XX__XX__XX__pingpong_entity_e_3_XX__XX__XX_Listener(){
	addedEntities['e_3_XX_'].forEach(function(item){item.angle += 180;}, this);
	r_1_XX_=r_1_XX_-8/2;
